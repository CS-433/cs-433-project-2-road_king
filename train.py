...
P_tr = torchvision.transforms.Compose([
    ...
    torchvision.transforms.Resize(size=608, interpolation=3)])
P_val = torchvision.transforms.Resize(size=608, interpolation=3)
...
n_f = 64
dropout = 0.2
model_cpu = UNet(n_channels=3, n_classes=1, n_filters=n_f)
model_gpu = model_cpu.to(device)
criterion = torch.nn.BCEWithLogitsLoss().to(device)
optimizer = torch.optim.Adam(model_gpu.parameters(), lr=LR, weight_decay=1e-6)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=5, verbose=True)
writer = SummaryWriter()
start_epoch = 0
end_epoch = 60
print('\n', '------------------------------------------------------------------')
print(f'Start traing process at epoch {start_epoch}, learning rate: {LR}, model: U-net-64, dropout={dropout}')
...
total_time = 0.0
iteration = 0
SAVE_PATH = r'/home/dguo/road_seg/checkpoints/Unet_{}_dice_dropout{}'.format(n_f, dropout)
score_min = 0.8
